...
describe(`API authenticate user if data is valid`, () => {
  ...
  test(`Status cond is 200`, () => expect(response.statusCode).toBe(HttpCode.OK));
  test(`User nam is Иван Иванов`, () => expect(response.body.name).toBe(`Иван Иванов`));
});

describe(`API resuse to authenticate user if data is invalid`, () => {
  let app;
